...
//				new LocalDateStringConverter(dtf, dtf));
	...
	public void populateTable() { 
		...
		TableColumn<Curso, Void> col5 = new TableColumn<>("Ações");
		Callback<TableColumn<Curso, Void>, TableCell<Curso, Void>> callBack = 
			new Callback<>() {
				@Override
				public TableCell<Curso, Void> call(final TableColumn<Curso, Void> param) {
					final TableCell<Curso, Void> tableCell = 
							new TableCell<Curso, Void>(){
						private final Button btnOk = new Button("Apagar");
						{
							btnOk.setOnAction( e -> {
								Curso c = table.getItems().get(getIndex());	
								control.apagar(c);
							});
						}
